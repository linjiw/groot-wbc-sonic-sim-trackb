...
def modality_json() -> dict:
    return {
        "state": {
            "observation.state": {"start": 0, "end": 43, "original_key": "observation.state"},
            "observation.eef_state": {"start": 43, "end": 55, "original_key": "observation.eef_state"},
...
def make_rows(episodes: int, frames: int, fps: int) -> list[dict]:
    rows: list[dict] = []
            ...
            rows.append(
                {
                    ...
                    "action.motion_token": (np.linspace(0.0, 1.0, 64, dtype=np.float32) + base).tolist(),
                    "teleop.left_hand_joints": (np.linspace(0.0, 0.2, 7, dtype=np.float32) + base).tolist(),
                    "teleop.right_hand_joints": (np.linspace(0.0, 0.2, 7, dtype=np.float32) + base).tolist(),
                    "teleop.smpl_pose": (np.linspace(0.01, 0.63, 63, dtype=np.float32) + base).tolist(),
                }
            )
    return rows


def create_fixture(output: Path, episodes: int, frames: int, fps: int) -> None:
    if output.exists():
    ...
    write_json(output / "meta" / "modality.json", modality_json())
    write_json(
        output / "meta" / "info.json",
        {
            ...
            "description": "Synthetic schema-only SONIC/VLA fixture; not for training.",
        },
    )
    ...
    df = pd.DataFrame(make_rows(episodes, frames, fps))
...
def main() -> int:
    ...
    create_fixture(args.output, args.episodes, args.frames, args.fps)
