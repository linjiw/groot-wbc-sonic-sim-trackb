...
def test_the_reported_frame_is_where_the_obstacle_sits():
    ...
    _, frame = swept_clearance_to_box(
        pos, quat, BODY_NAMES, capsules=TEST_CAPSULES, box=shelf(1.0).box_at(0.0)
    )
    assert 24 <= frame <= 36
...
def test_the_boundary_search_finds_the_zero_crossing():
    ...
    def clearance_at(parameter):
        return swept_clearance_to_box(
            pos, quat, BODY_NAMES, capsules=TEST_CAPSULES, box=spec.box_at(parameter)
        )
...
def test_a_built_family_straddles_the_boundary():
    pos, quat = straight_walk()
    family = build_family(
        "f0", pos, quat, BODY_NAMES, shelf(3.0), search_high=1.9, capsules=TEST_CAPSULES
    )
...
def test_a_lower_body_needs_a_lower_shelf_to_be_hit():
    ...
    def boundary_for(pos, quat):
        return find_collision_boundary(
            lambda p: swept_clearance_to_box(
                pos, quat, BODY_NAMES, capsules=TEST_CAPSULES, box=spec.box_at(p)
            ),
            0.0,
            1.9,
            tolerance_m=0.002,
...
def test_the_window_is_the_family_content():
    ...
    narrow = build_paired_family(
        "narrow",
        (tall_pos, tall_quat, BODY_NAMES),
        (shallow_pos, shallow_quat, BODY_NAMES),
        shelf(3.0),
        search_high=2.4,
        capsules=TEST_CAPSULES,
    )
