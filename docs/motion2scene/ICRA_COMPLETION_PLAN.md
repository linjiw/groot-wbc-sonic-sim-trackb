...
**Current implementation:** use the frozen [independent-layout protocol](INDEPENDENT_LAYOUT_EXECUTION_V1.md) and [methods draft](LEARNING_COMPARISON_METHODS_DRAFT.md). Twenty supervised outcome predictors are fitted at eleven encounters per arm; the first 120/600 policy executions are measured. The primary completed-panel comparator is Motion2Scene minus analytic passage. The older general design below is retained as planning history; it does not replace the implemented endpoint or imply completed source transfer.

### Earlier broad design and remaining scope
...
analysis. This gate has a partial development comparison; the learning-data advantage and final source-transfer evidence remain unproved.
